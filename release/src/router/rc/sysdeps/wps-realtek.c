...
int start_wps_pin_method(WPS_MODE mode,char* pin)
{
	...
	{
		if (nvram_get_int("sw_mode") == SW_MODE_AP && nvram_get_int("wlc_psta") == 1) { // media bridge mode.
...
int start_wps_pbc_method(WPS_MODE mode)
{
	...
	{
#ifdef RTCONFIG_CONCURRENTREPEATER
		if (nvram_get_int("sw_mode") == SW_MODE_REPEATER && nvram_get_int("wlc_express") != 0) { // Express mode
			...
			if (nvram_get_int("sw_mode") == SW_MODE_AP && nvram_get_int("wlc_psta") == 1) // media bridge
				fp = fopen("/var/run/wscd-wl0.pid","r");
			else
...
start_wps_method(void)
{
	...
	sleep(1); //Wait for update tmp/wscd_status
...
#endif
	if (nvram_get_int("sw_mode") == SW_MODE_REPEATER || (nvram_get_int("sw_mode") == SW_MODE_AP && nvram_get_int("wlc_psta") == 1))
#ifdef RPAC53
