...
{	// return WLC connection status
	...
	if (nvram_get_int("sw_mode") == SW_MODE_REPEATER && wlc_express == 0) {
...
#define WSC_WPA2_AES		8
	...
	if(sw_mode != 2 && (sw_mode == 3 && wlc_psta != 1))/*not repeater mode and media bridge mode */
	{
	    if(wlanif_name != NULL) {
	...
	nvram_ptr = nvram_get("wps_device_name");
...
#if 1
	if ((sw_mode == 2 && client_mode) || (sw_mode == 3 && wlc_psta == 1 && client_mode))
	{
