...
#define CPU_CLOCK_RATE	360000000UL
#else
...
#if BAUDRATE == 57600 || BAUDRATE == 115200
...
/* stage1 only need to handle stage2, reduce malloc length to save boot time */
#define CFG_MALLOC_LEN		(2 * 1048576UL)
#else
#define CFG_MALLOC_LEN		(16 * 1048576UL)
#endif
...
#define	CFG_LOAD_ADDR		(TEXT_BASE + 0x1000000)	/* default load address	*/
#else
...
#else
#define MAX_LOAD_LENGTH		(16 * 1048576UL)
#endif
...
#define CFG_SPINAND_LOAD_ADDR	0x81000000
#else
...
#define MTDPARTS		"mtdparts=nand0:896k(Bootloader),128k(environment),-(UBI_DEV)"
...
#define CFG_UBI_FIRMWARE_NR_LEB		265	/* 124KB x 265 = 30.089MB */
#define CFG_UBI_FIRMWARE2_NR_LEB	265
#define CFG_UBI_APP_NR_LEB		483	/* 124KB x 483 = 58.488MB. This volume size cannot reach requested size due to UBI's overhead. */
...
#if defined(CFG_ENV_IS_IN_NAND)
#define CFG_BOOTLOADER_SIZE	0xE0000	/* 128KB boot strap + 3 x 256KiB bootloader  = 896k */
#define CFG_MAX_BOOTLOADER_BINARY_SIZE	0x40000	/* bootloader size is limited under 256KiB */
#define CFG_CONFIG_SIZE		0x20000
#define CFG_BOOTSTAGE1_SIZE	0x20000									/* 2 STAGE */
#define CFG_BOOTSTAGE2_SIZEALL	(CFG_BOOTLOADER_SIZE - CFG_BOOTSTAGE1_SIZE)				/* 2 STAGE */
#define CFG_BOOTSTAGE2_OFFSET	(CFG_BOOTSTAGE1_SIZE)							/* 2 STAGE */
#define CFG_BOOTSTAGE2_ADDR	(CFG_FLASH_BASE + CFG_BOOTSTAGE2_OFFSET)				/* 2 STAGE */
#if defined(UBI_SUPPORT)
#define CFG_FACTORY_SIZE	(CFG_UBI_FACTORY_SIZE + CFG_UBI_FACTORY2_SIZE)
#else
#define CFG_FACTORY_SIZE	((128 * 1024 * 2) * 2)  //0x80000, 2 Block(128 * 1024) per Factory area, totally 2 Factory area.
#endif
#else
