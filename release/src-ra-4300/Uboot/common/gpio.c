...
#if defined(ASUS_RTN14U) || defined(ASUS_RTAC52U) || defined(ASUS_RTAC51U) || defined(ASUS_RTAC51UP) || defined(ASUS_RTAC53) || defined(ASUS_RTN11P) || defined(ASUS_RTN54U)
...
#if defined (ASUS_RPAC56) || defined(ASUS_RTAC1200GA1) || defined(ASUS_RTAC1200GU)
//MT7621
//asm/rt2880/rt_mmap.h
...
#if defined(ASUS_RTAC1200) || defined(ASUS_RTN11PB1)
//asm/rt2880/rt_mmap.h
...
#else
#error Invalid Product!!
#endif //#if defined(ASUS_RTAC1200) || defined(ASUS_RTN11PB1)
...
#endif   
{
	...
	if (shift >= 0) {
		...
		unsigned int used_gpiomode_reg = RT2880_GPIOMODE_REG;

		if (gpio2)
			used_gpiomode_reg = RT2880_GPIO2MODE_REG;

		old = ra_inl(used_gpiomode_reg);

		ra_and(used_gpiomode_reg, ~(mask << shift));
		if (val)
			ra_or(used_gpiomode_reg, val << shift);

		if (old != (new = ra_inl(used_gpiomode_reg))) {
...
void led_init(void)
{
	...
	int i, led[] = { PWR_LED, WIFI_2G_LED, WAN_LED, n, USB_LED };
	...
	int i, led[] = { PWR_LED, WIFI_2G_LED, WAN_LED, LAN_LED };
	...
	for (i = 0; i < ARRAY_SIZE(led); ++i) {
			...
			//else
			//	set_gpio_pin(led[i], 1);	/* turn off LED */
...
void led_all_off(){
	...
	for (i = 0; i < ARRAY_SIZE(led); ++i) {
			set_gpio_pin(led[i], 1);	/* turn off ALL LED */
...
void led_all_on(){
	...
	for (i = 0; i < ARRAY_SIZE(led); ++i) {
			if (led[i] != PWR_LED)
				set_gpio_pin(led[i], 0);	/* turn on WiFi WAN LAN LED */
			else
				set_gpio_pin(led[i], 1);	/* turn off power LED */
...
void led_power_on(){
	...
	for (i = 0; i < ARRAY_SIZE(led); ++i) {
			if (led[i] == PWR_LED)
				set_gpio_pin(led[i], 0);	/* turn on power LED */
	}
}
...
extern const char *model;
	...
	printf("ASUS %s gpio init : wps / reset pin / reset switch pin\n", model);
	mtk7620_set_gpio_dir(WPS_BTN, 1);
	mtk7620_set_gpio_dir(RST_BTN, 1);
	mtk7620_set_gpio_dir(RESET_SWITCH, 0);
	mtk7620_set_gpio_pin(RESET_SWITCH, 1); /* Pull high */
#elif defined(ASUS_RTN54U)
	printf("ASUS %s gpio init : wps / reset pin\n", model);
	mtk7620_set_gpio_dir(WPS_BTN, 1);
	mtk7620_set_gpio_dir(RST_BTN, 1);
...
void LEDON(void)
{
	...
	set_gpio_pin(WAN_LED, 0);
	set_gpio_pin(LAN_LED, 0);	
#elif defined(ASUS_RTAC1200)
	set_gpio_pin(PWR_LED, 0);
