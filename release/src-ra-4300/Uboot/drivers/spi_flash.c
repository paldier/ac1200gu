...
int spic_init(void)
{
	...
	ra_outl(RT2880_SPI0_CFG_REG, SPICFG_MSBFIRST | SPICFG_TXCLKEDGE_FALLING | SPICFG_TXCLKEDGE_FALLING | CFG_CLK_DIV| SPICFG_ADDRMODE); 
#else
...
static struct chip_info chips_data [] = {
	...
	{ "25Q64BSIG",		0xc8, 0x4017c840, 64 * 1024, 128, 0 },
	{ NULL,                 0x00, 0x00000000, 0,  0 },
};
...
#ifdef SPI_PROBE_IN_CMD
struct chip_info *chip_prob2(void)
{
	...
	weight = 0xffffffff;
	match = &chips_data[0];
	for (i = 0; (i < sizeof(chips_data)/sizeof(chips_data[0]))&&(chips_data[i].name!=NULL); i++) {
		info = &chips_data[i];
		if (info->id == buf[0]) {
			if (info->jedec_id == jedec) {
				printf("find flash: name_p[%p],id:[%02x],jdec:[%08x],sector:[%08x];nsector[%d]\n", info->name,info->id,info->jedec_id,info->sector_size,info->n_sectors);
				return info;
			}
...
#ifdef SPI_PROBE_IN_CMD
unsigned long raspi_init2(void)
{
	spic_init();
	spi_chip_info = chip_prob2();
...
int do_mem_cp(cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	if (!strncmp(argv[0], "cp.linux", 9)) {
...
#ifdef SPI_PROBE_IN_CMD
	else if (!strncmp(argv[0], "cp.probe", 9)) {
	        if ((offset = raspi_init2()) == (ulong)-1)
	                printf("ra_spi_init2 fail\n");
		return 0;
	}
	else if (!strncmp(argv[0], "cp.rspi", 8)) {
		if (argc>3)
			addr = simple_strtoul(argv[3], NULL, 16);
		if (argc>2) {
			offset = simple_strtoul(argv[1], NULL, 16);
			count = simple_strtoul(argv[2], NULL, 16);
			raspi_read(addr, offset, count);
			NetBootFileXferSize = count;
			printf("\n Read [%d byte] from SPI offset[0x%08X] to [0x%08x]\n", count, offset, addr);
			return 0;
		} else {
			printf ("Usage:\n%s\n", cmdtp->usage);
			return 1;
		...
		if (argc>1) {
			...
			printf("\n Normal Copy [%d byte] to SPI Flash[0x%08X] from[0x%08x]?\n", count, offset, addr);
			ra_flash_erase_write((uchar*) addr, CFG_FLASH_BASE+offset, count, 0);
			printf("\ndone\n");
		} else {
#endif
		printf ("Usage:\n%s\n", cmdtp->usage);
...
#ifdef SPI_PROBE_IN_CMD
	"cp.probe\n    - probe spi flash again\n"
	"cp.rspi offset len [address]\n    - read spi offset len bytes to [address]\n"
	"cp offset [address]\n    - write tftp data into flash offset from [address]\n"
#endif
);
...
int do_flerase (cmd_tbl_t *cmdtp, int flag, int argc, char *argv[])
{
	...
	else if (strcmp(argv[1], "env") == 0)
	{
	         ...
	         raspi_unprotect();
	         rcode = raspi_erase(CFG_ENV_ADDR - CFG_FLASH_BASE, CFG_CONFIG_SIZE);
	         return rcode;
	}
