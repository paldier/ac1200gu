...
#else
static inline void ctrl_mdc_clock(int enable) { }
#endif
...
void mii_mgr_init(void)
{
	...
	/* Disable MDC clock by default */
	data |= GPIO_MDIO_BIT;
	outw(GPIO_PRUPOSE, data);
}
#endif /* SWITCH_CHIP_RTL8367RB */
...
U_BOOT_CMD(
 	...
 	"mdio.wb phy register bit offset Value(0/1)  \n"
 	"mdio.d - dump all Phy registers \n"
 	"mdio.d phy register - dump Phy registers \n"
);
