...
#define WAN_PORT_MASK	0x1		//WLLLL, wan at P0, demo board
#endif
#ifdef RALINK_EV_BOARD_PVLAN
#define WAN_PORT_MASK	0x10	//LLLLW, wan at P4, ev board
#endif

#if defined(ASUS_RTN11PB1)
static int last_lan_status = 0;
static int last_wan_status = 0;
void check_eth_link_status()
{
	unsigned int esw_poa = (RALINK_REG(REG_ESW_POA) >> 25) & 0x1f;
	int cur_lan_status = (esw_poa & ~(WAN_PORT_MASK)) ? 1 : 0;
	int cur_wan_status = (esw_poa & WAN_PORT_MASK) ? 1 : 0;

	if (cur_lan_status != last_lan_status) {
		
		last_lan_status = cur_lan_status;
		set_gpio_pin(LAN_LED, cur_lan_status ? 0 : 1);
	}

	if (cur_wan_status != last_wan_status) {
		last_wan_status = cur_wan_status;
		set_gpio_pin(WAN_LED, cur_wan_status ? 0 : 1);
	}
}
#endif
#endif
