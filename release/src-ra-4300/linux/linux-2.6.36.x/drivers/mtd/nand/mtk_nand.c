...
#error "No cpde to handle this case in MTK NAND Driver..."
    ...
    MSG(INIT, "[mtk_nand] Line = %d MTD_BOOT_PART_SIZE = %08X  MTD_CONFIG_PART_SIZE= %08X  MTD_FACTORY_PART_SIZE=%08X!\n", __LINE__, MTD_BOOT_PART_SIZE, MTD_CONFIG_PART_SIZE, MTD_FACTORY_PART_SIZE);

    if (rfs_offset != 0) {
    MSG(INIT, "[mtk_nand] Line = %d!\n", __LINE__);
        //g_pasStatic_Partition[4].offset = MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE + rfs_offset;
    ...
    MSG(INIT, "[mtk_nand] Line = %d!\n", __LINE__);
          //  g_pasStatic_Partition[3].size = mtd->size - (MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE) - MTD_JFFS2_PART_SIZE;
