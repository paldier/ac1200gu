...
//asus #if defined (CONFIG_RT2880_ROOTFS_IN_FLASH) && defined (CONFIG_ROOTFS_IN_FLASH_NO_PADDING)
	...
	if (rfs_offset != 0) {
		...
		if (flash->mtd.size > 0x800000) {
			...
			rt2880_partitions[3].size = flash->mtd.size - (MTD_BOOT_PART_SIZE + MTD_CONFIG_PART_SIZE + MTD_FACTORY_PART_SIZE) - CONFIG_MTD_JFFS2_LEN;
			rt2880_partitions[4].size = flash->mtd.size - rt2880_partitions[4].offset - CONFIG_MTD_JFFS2_LEN;
			if (CONFIG_MTD_JFFS2_LEN > 0) {
				rt2880_partitions[5].offset = flash->mtd.size - CONFIG_MTD_JFFS2_LEN;
				rt2880_partitions[5].size = CONFIG_MTD_JFFS2_LEN;
			}
			else
			{
