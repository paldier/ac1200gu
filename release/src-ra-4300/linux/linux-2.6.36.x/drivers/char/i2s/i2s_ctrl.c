...
int i2s_mmap_alloc(struct vm_area_struct *vma, unsigned long size)
{
	int i;
	u32 page_size;
       	int first_index;
...
#if !defined(CONFIG_SND_RALINK_SOC)
	printk("MMAP[%d]=0x%08X, vm_start=%08X,vm_end=%08X, i2s_mmap_addr[%d]=0x%08x\n",
		pi2s_config->mmap_index, (u32)pi2s_config->pMMAPBufPtr[pi2s_config->mmap_index], 
		(u32)vma->vm_start, (u32)vma->vm_end, pi2s_config->mmap_index, 
		i2s_mmap_addr[pi2s_config->mmap_index]);
#endif
...
{
	int xferIdx = pi2s_config->tx_r_idx;
	if(ptri2s_config->bALSAEnable && pi2s_config->bALSAMMAPEn)
		xferIdx = (xferIdx + HW_PTR_SHIFT)%MAX_I2S_PAGE;
	if(dma_ch==GDMA_I2S_TX0)
        {
#if defined(CONFIG_I2S_MMAP)
		dma_sync_single_for_device(NULL,  i2s_mmap_addr[xferIdx], I2S_PAGE_SIZE, DMA_TO_DEVICE);
                GdmaI2sTx((u32)(pi2s_config->pMMAPTxBufPtr[xferIdx]), I2S_TX_FIFO_WREG, 0, I2S_PAGE_SIZE, i2s_dma_tx_handler, i2s_dma_tx_unmask_handler);
#else
                memcpy(pi2s_config->pPage0TxBuf8ptr,  pi2s_config->pMMAPTxBufPtr[xferIdx], I2S_PAGE_SIZE);
...
#if defined(CONFIG_I2S_MMAP)
		dma_sync_single_for_device(NULL,  i2s_mmap_addr[xferIdx], I2S_PAGE_SIZE, DMA_TO_DEVICE);
                GdmaI2sTx((u32)(pi2s_config->pMMAPTxBufPtr[xferIdx]), I2S_TX_FIFO_WREG, 1, I2S_PAGE_SIZE, i2s_dma_tx_handler, i2s_dma_tx_unmask_handler);
#else
                memcpy(pi2s_config->pPage1TxBuf8ptr,  pi2s_config->pMMAPTxBufPtr[xferIdx], I2S_PAGE_SIZE);
...
{
	int xferIdx = pi2s_config->rx_w_idx = (pi2s_config->rx_w_idx+1)%MAX_I2S_PAGE;
	...
	if(dma_ch==GDMA_I2S_RX0)
        {
#ifdef CONFIG_I2S_MMAP
                dma_sync_single_for_device(NULL,  i2s_mmap_addr[xferIdx+(pi2s_config->mmap_index-MAX_I2S_PAGE)], I2S_PAGE_SIZE, DMA_FROM_DEVICE);
                GdmaI2sRx(I2S_RX_FIFO_RREG, (u32)(pi2s_config->pMMAPRxBufPtr[xferIdx]), 0, I2S_PAGE_SIZE, i2s_dma_rx_handler, i2s_dma_rx_unmask_handler);
#else
                memcpy(pi2s_config->pMMAPRxBufPtr[xferIdx], pi2s_config->pPage0RxBuf8ptr, I2S_PAGE_SIZE);
...
#ifdef CONFIG_I2S_MMAP
                dma_sync_single_for_device(NULL,  i2s_mmap_addr[xferIdx+(pi2s_config->mmap_index-MAX_I2S_PAGE)], I2S_PAGE_SIZE, DMA_FROM_DEVICE);
                GdmaI2sRx(I2S_RX_FIFO_RREG, (u32)(pi2s_config->pMMAPRxBufPtr[xferIdx]), 1, I2S_PAGE_SIZE, i2s_dma_rx_handler, i2s_dma_rx_unmask_handler);
#else
                memcpy(pi2s_config->pMMAPRxBufPtr[xferIdx], pi2s_config->pPage1RxBuf8ptr, I2S_PAGE_SIZE);
		...
		if(aecFuncP->AECNeEnq){
			aecFuncP->AECNeEnq(0,pi2s_config->pMMAPRxBufPtr[xferIdx],I2S_PAGE_SIZE);
		}
#endif
	return 0;
}
...
void i2s_dma_rx_handler(u32 dma_ch)
{
	...
	if(pi2s_config->bALSAEnable){
		if(pi2s_config->dmaStat[SNDRV_PCM_STREAM_CAPTURE]){
			if(!pi2s_config->bTrigger[1]){
...
void i2s_dma_rx_end_handle(i2s_config_type* ptri2s_config)
{
...
#if defined(CONFIG_I2S_MMAP)
        	i2s_mmap_alloc(NULL,I2S_TOTAL_PAGE_SIZE);
#endif
        	i2s_txbuf_alloc(ptri2s_config);
...
#if defined (CONFIG_I2S_MMAP) && defined (CONFIG_SND_RALINK_SOC)
	pi2s_config->bALSAMMAPEn = 1;
#endif
    init_waitqueue_head(&(pi2s_config->i2s_tx_qh));
    init_waitqueue_head(&(pi2s_config->i2s_rx_qh));
	spin_lock_init(&pi2s_config->lock);
...
EXPORT_SYMBOL(i2s_dma_rx_end_handle);
