...
static inline int mdc_mdio_write(rtk_uint32 preamble_len, rtk_uint32 phy_id, rtk_uint32 register_id, rtk_uint32 data)
{
	int r;
...
#else
static inline int _rtl865x_initGpioPin(int gpioIdx, int dummyParam0, int gpioDir, int dummyParam1)
{
	ralink_initGpioPin(gpioIdx, gpioDir);
	return RT_ERR_OK;
}

static inline void _rtl865x_setGpioDataBit(int gpioIdx, unsigned int val)
{
	ralink_gpio_write_bit(gpioIdx, val);
}

static inline void _rtl865x_getGpioDataBit(int gpioIdx, unsigned int* val)
{
	ralink_gpio_read_bit(gpioIdx, val);
}

static gpioID GPIO_ID(int port, gpioID pin)
{
	return pin;
}
