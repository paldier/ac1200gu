...
static int wm8960_trigger(struct snd_pcm_substream *substream, int cmd,struct snd_soc_dai *dai){
     int ret = 0;
...
#if 0
		if (substream->stream == SNDRV_PCM_STREAM_PLAYBACK){
			reg = snd_soc_read(codec, WM8960_LOUT1) &0xff;
			printk("LOUT1:%s \n",reg);
			if(reg == 0)
				snd_soc_update_bits(codec, WM8960_LOUT1, 0x17f, 0x1f8);

			reg = snd_soc_read(codec, WM8960_ROUT1) &0xff;
			if(reg == 0)
				snd_soc_update_bits(codec, WM8960_ROUT1, 0x17f, 0x1f8);

			reg = snd_soc_read(codec, WM8960_LOUT2) &0xff;
			if(reg == 0)
				snd_soc_update_bits(codec, WM8960_LOUT2, 0x17f, 0x1f8);

			reg = snd_soc_read(codec, WM8960_ROUT2) &0xff;
			if(reg == 0)
				snd_soc_update_bits(codec, WM8960_ROUT2, 0x17f, 0x1f8);
		}
		else{
			reg = snd_soc_read(codec, WM8960_LINVOL) &0xff;
			if(reg == 0)
				snd_soc_update_bits(codec, WM8960_LINVOL, 0x13f, 0x120);

			reg = snd_soc_read(codec, WM8960_RINVOL) &0xff;
			if(reg == 0)
				snd_soc_update_bits(codec, WM8960_RINVOL, 0x13f, 0x120);
		}
#endif
         break;
